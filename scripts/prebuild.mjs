...
function clashMetaAlpha() {
  ...
  return {
    name: 'sherry-core-alpha',
    targetFile: `sherry-core-alpha-${SIDECAR_HOST}${isWin ? '.exe' : ''}`,
...
function clashMeta() {
  ...
  return {
    name: 'sherry-core',
    targetFile: `sherry-core-${SIDECAR_HOST}${isWin ? '.exe' : ''}`,
    exeFile: `${name}${isWin ? '.exe' : ''}`,
...
const resolveServicePermission = async () => {
  const serviceExecutables = [
    'sherry-service*',
    'sherry-service-install*',
    'sherry-service-uninstall*',
  ]
  const hashCache = await loadHashCache()
...
function serviceFileInfo(name) {
  ...
  const targetName = name.replace('clash-verge-service', 'sherry-service')
  return {
    sourceFile: `${name}${ext}`,
    targetFile: `${targetName}${suffix}${ext}`,
  }
}
